...
* <u>**CTONet**</u>
    * **简要说明：**  这篇论文提出了一种名为 **CTO** (Convolution, Transformer, and Operator) 的新型网络架构，旨在解决医学图像分割中**边界区域分割不准确**的难题 。该架构采用了双流编码器设计，结合了用于提取局部特征的主流 **CNN** 和用于捕捉长距离依赖的高效 **StitchViT**；其核心创新在于引入了**边界引导解码器**，利用显式的边缘检测算子（如 Sobel）生成边界掩码，通过边界注入模块（BIM）为解码过程提供明确的监督信号，从而显著提升了模型对模糊边界的识别能力 。在 ISIC、CoNIC、LiTS17 和 BTCV 等 **7 个主流医学图像分割数据集**上的实验表明，CTO 在保持计算效率的同时，均取得了优于现有 SOTA 方法的分割精度。
    * **链接:** [[Paper]](https://arxiv.org/pdf/2505.04652v1) [[Code]](https://github.com/xiaofang007/CTO) (MeDIA 2025)
...
* <u>**Strip R-CNN**</u>
    * **简要说明：** 这篇论文提出了 **Strip R-CNN**，一种专门针对遥感图像中高长宽比（细长）物体检测难题而设计的高效网络架构 。针对传统方形大核卷积在处理细长物体时易引入背景噪声且效率低下的问题 ，作者设计了 **StripNet** 主干网络，利用序列化的水平和垂直大尺寸条状卷积（Large Strip Convolutions）来高效捕捉各向异性的长距离上下文信息。此外，论文还改进了检测头设计，提出了 **Strip Head**，通过解耦定位与分类任务并在定位分支引入条状卷积，显著增强了对细长物体的定位精度和角度回归能力 。实验结果表明，Strip R-CNN 在 DOTA-v1.0 数据集上取得了 82.75% mAP 的 SOTA 成绩，并在 FAIR1M、HRSC2016 等多个基准上优于现有方法，且模型参数量和计算量更低 。
    * **链接:** [[Paper]](https://arxiv.org/abs/2501.03775) [[Code]](https://github.com/HVision-NKU/Strip-R-CNN) ( 2025)

* <u>**PConv SDLoss**</u>
    * **简要说明：**这篇论文提出了一种新的红外小目标检测方法，旨在解决现有 CNN 方法忽略红外小目标高斯空间分布特征的问题 。为此，作者引入了一种即插即用的风车形卷积（PConv）模块，该模块通过不对称填充和分组卷积，更好地适应小目标的像素分布，显著扩大了感受野，并增强了底层特征提取能力，且参数量增加极小 。针对现有损失函数未能充分考虑不同尺度目标对尺度和位置损失敏感性差异的问题，论文提出了基于尺度的动态（SD）损失函数，根据目标大小动态调整损失权重，从而提升对不同尺度目标的回归能力 。此外，作者构建了一个新的、更具挑战性的红外小目标检测数据集 SIRST-UAVB，包含大量无人机和鸟类目标 。实验结果表明，该方法在 IRSTD-1K 和 SIRST-UAVB 数据集上均取得了显著的性能提升，验证了其有效性和通用性 。
    * **链接:** [[Paper]](https://arxiv.org/pdf/2412.16986) [[Code]](https://github.com/JN-Yang/PConv-SDloss-Data) (AAAI 2025)
